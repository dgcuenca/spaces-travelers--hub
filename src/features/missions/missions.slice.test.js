...
import { fetchMissions, getJoinedMissions, joinMission } from './missions.slice';
...
describe('missions redux state tests', () => {
  ...
  it('Should be able to fetch the missions list', async () => {
    const result = await store.dispatch(fetchMissions());
    expect(result.type).toBe('missions/FETCH_MISSIONS/fulfilled');
    const state = store.getState().missions;
  ...
  it('Falcon 1 Should be reserved in missions list', () => {
    store.dispatch(joinMission('9D1B7E0'));
    const state = store.getState().missions;
    expect(state.missions[0].mission_name).toEqual('Thaicom');
    expect(state.missions[0].reserved).toEqual(true);
  });

  it('Starship Should be reserved in missions list', () => {
    store.dispatch(joinMission('F4F83DE'));
    const state = store.getState().missions;
    expect(state.missions[1].mission_name).toEqual('Telstar');
    expect(state.missions[1].reserved).toEqual(true);
  });

  it('Show only reserved missions list', () => {
    const state = getJoinedMissions(store.getState());
